 ...
 * サポートイベント一覧コンポーネント
 *
 * サポート詳細モーダル内でサポートイベントを解放順に一覧表示する。
        ...
        <div key={i} className={`flex items-start gap-3 p-3 rounded-xl ${colors.bg} border ${colors.border}`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {evt.effect_type === EventEffectType.SkillCard &&
                card.skill_card &&
                (() => {
                  ...
                  return (
                    <>
                      <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${rEntry.color}`}>
                        {t(rEntry.label)}
                      </span>
                      <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${sEntry.badge}`}>
                        {t(sEntry.label)}
                      </span>
                    </>
                  )
