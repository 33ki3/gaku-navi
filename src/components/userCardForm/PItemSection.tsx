  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="bg-slate-50 rounded-lg p-3 border border-slate-200 flex flex-col gap-3">
        {/* 発動条件 */}
        <div>
          <label className={constant.USER_FORM_SECTION_LABEL}>{t('user_support.pitem_trigger')}</label>
          <select
          ...
          {validation.pItemTriggerError && (
            <p className="text-xs text-red-500 mt-1">
              {t(data.getFormErrorTranslationKey(validation.pItemTriggerError))}
            </p>
          )}
        </div>
        ...
        {!isNoneTrigger && (
          <>
            {/* パラメータ上昇値 */}
            <div>
              <label className={constant.USER_FORM_SECTION_LABEL}>{t('user_support.pitem_value')}</label>
              <input
                type="text"
                inputMode="numeric"
            ...
            {/* 発動回数 */}
            <div>
              <label className={constant.USER_FORM_SECTION_LABEL}>{t('user_support.pitem_effect_count')}</label>
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  placeholder={t('user_support.pitem_effect_count_placeholder')}
            ...
            {/* 発動時効果リスト */}
            <div>
              <label className={constant.USER_FORM_SECTION_LABEL}>{t('user_support.pitem_actions')}</label>
              <div className="flex flex-col gap-2">
                {form.pItemEffects.map((effect, i) => (
                  <div key={i} className="bg-white rounded-lg p-2.5 border border-slate-200 relative">
                    ...
                    >
                      {t('user_support.pitem_remove_effect')}
                    </button>
                    {/* 条件（効果タイプ） */}
                    <div>
                      <label className="text-[10px] text-slate-400">{t('user_support.pitem_effect_type')}</label>
                      <select
                        value={effect.action}
                        onChange={(e) => {
                    ...
                    <div className="mt-1.5">
                      <label className="text-[10px] text-slate-400">{t('user_support.pitem_effect_body_count')}</label>
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => {
              ...
              {validation.pItemBodyCountError && form.pItemEffects.some((e) => !e.count) && (
                <p className="text-xs text-red-500 mt-1">
                  {t(data.getFormErrorTranslationKey(validation.pItemBodyCountError))}
                </p>
              )}
              {/* 追加ボタン */}
              <button
