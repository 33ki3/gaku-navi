...
/** 実サポートデータとアクション回数を使った手計算との照合テスト */
describe('統合テスト: 実サポート × アクション回数', () => {
...
describe('統合テスト: スケジュール → パラメータボーナス → 点数計算', () => {
  ...
  it('extraEventCounts が他サポート由来の追加回数として加算される', () => {
    // ぐつぐつ (uncap4):
...
describe('統合テスト: 実スケジュール × 実サポート', () => {
  describe('パターン1: Vo重視（Vo×4 + Da×1）', () => {
    ...
    it('2名様ご案内: Pアイテム ON（元気スキルカード獲得で Vo+6）', () => {
...
/** concentration_card_acquire テスト用サポート */
  ...
  events: [
    {
      ...
      param_type: enums.ParameterType.Dance,
      param_value: 10,
      title: 'mock event',
    },
  ],
...
/** motivation_card_acquire テスト用サポート */
  ...
  events: [
    {
      release: enums.ReleaseConditionType.Initial,
      effect_type: enums.EventEffectType.ParamBoost,
      param_type: enums.ParameterType.Visual,
      param_value: 15,
      title: 'mock event',
    },
  ],
...
describe('統合テスト: モックサポート（未テストアビリティ網羅）', () => {
  it('concentration_card_acquire: 凸4 集中スキルカード獲得×4', () => {
  ...
  it('concentration_card_acquire: 凸0 集中スキルカード獲得×2', () => {
    // initial_stat: 20
  ...
  it('motivation_card_acquire: 凸4 やる気スキルカード獲得×4 + SPレッスン×3', () => {
    // initial_stat: 50
  ...
  it('motivation_card_acquire: 凸0 やる気スキルカード獲得×1', () => {
    // initial_stat: 25
  ...
  it('reserve_card_acquire: 凸4 温存スキルカード獲得×4 + paramBonus', () => {
  ...
  it('reserve_card_acquire: 凸0 温存スキルカード獲得×3 + paramBonus', () => {
