...
function App() {
  ...
  // CardList・CardListItem で共有するサポート操作コンテキストを生成
  const cardCtx = useMemo<CardContextValue>(
    () => ({
      ...
      onCardClick: (card) => {
        // サポート一覧選択モード中はサポートを手動編成に追加する
        if (state.ui.unitCardSelectMode && addManualCardRef.current) {
          addManualCardRef.current(card.name)
          return
        }
        state.handlers.handleCardClick(card)
      },
      ...
      isCardEligible: (card) => {
        if (!state.ui.unitCardSelectMode) return true
        return isCardEligibleRef.current ? isCardEligibleRef.current(card) : true
      },
    }),
    [state.scores.getCardUncap, state.ui.uncapEditMode, state.ui.unitCardSelectMode, state.handlers],
  )
  ...
  return (
    // サポート操作コンテキストをアプリ全体に提供
    <CardProvider value={cardCtx}>
      <div
        className={`min-h-screen bg-[#f8fafc] text-slate-900 font-sans transition-all duration-300 ${state.ui.bothPanelsPinned ? 'md:pr-[48rem]' : state.ui.anyPanelPinned ? 'md:pr-96' : ''}`}
      >
        {/* ヘッダー（タイトル・アクションボタン） */}
        <AppHeader
